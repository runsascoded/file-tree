/** Convenience re-export. Most consumers import directly from
 *  `@rdub/file-tree/stores/r2` etc. to keep their dep graph narrow. */
export { R2Store } from './r2'
export type { R2StoreOptions } from './r2'
export { HttpStore } from './http'
export type { HttpStoreOptions } from './http'
export { MockStore } from './mock'
export type { MockStoreInput, MockStoreFile, MockStoreOptions } from './mock'
export { MultiStore } from './multi'
export type { MultiStoreInput } from './multi'
export { S3Store } from './s3'
export type { S3StoreOptions } from './s3'
export { GcsStore } from './gcs'
export type { GcsStoreOptions } from './gcs'
