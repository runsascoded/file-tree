...
__export(conformance_exports, {
  ...
  runStoreConformance: () => runStoreConformance
});
...
async function listAll(store, prefix) {
  ...
  let cursor = void 0;
  for (let i = 0; i < 100; i++) {
    const r2 = await store.list(prefix, cursor ? { cursor } : void 0);
    out.push(...r2.entries);
    if (!r2.cursor) return out;
    cursor = r2.cursor;
  }
...
function runStoreConformance(makeStore, opts = {}) {
  describe("Store conformance", () => {
    it("lists root with delimiter grouping", async () => {
      ...
      globalExpect(dirs).toEqual(["data/", "docs/"]);
      globalExpect(files).toEqual(["README.md", "binary.bin"]);
    });
    it("lists nested prefix", async () => {
      ...
      const dirs = all.filter((e) => e.isDir).map((e) => e.key).sort();
      const files = all.filter((e) => !e.isDir).map((e) => e.key).sort();
      globalExpect(dirs).toEqual(["docs/guide/"]);
      globalExpect(files).toEqual(["docs/intro.md"]);
    });
    it("lists deepest leaf prefix", async () => {
      const store = await makeStore();
      const all = await listAll(store, "data/2024/");
      const files = all.filter((e) => !e.isDir).map((e) => e.key).sort();
      globalExpect(files).toEqual(["data/2024/q1.csv", "data/2024/q2.csv"]);
    });
    it("returns size + lastModified for files", async () => {
      const store = await makeStore();
      const all = await listAll(store, "");
      const readme = all.find((e) => e.key === "README.md");
      globalExpect(readme).toBeTruthy();
      globalExpect(readme.size).toBeGreaterThan(0);
      globalExpect(readme.lastModified).toMatch(/^\d{4}-\d{2}-\d{2}T/);
    });
    if (!opts.skipPagination) {
      it("paginates with cursor when there are more entries than fit", async () => {
        ...
        let cursor = void 0;
        for (let i = 0; i < 100; i++) {
          const r2 = await store.list("", cursor ? { cursor } : void 0);
          collected.push(...r2.entries.map((e) => e.key));
          if (!r2.cursor) {
            cursor = void 0;
            break;
          }
        ...
        globalExpect(cursor).toBeUndefined();
        globalExpect(new Set(collected)).toEqual(/* @__PURE__ */ new Set(["data/", "docs/", "README.md", "binary.bin"]));
      });
    }
    it("gets text file as bytes", async () => {
      const store = await makeStore();
      const r2 = await store.get("README.md");
      const text = new TextDecoder().decode(r2.bytes);
      globalExpect(text).toBe("# fixture\n\nTop-level readme.\n");
      globalExpect(r2.totalSize).toBe(r2.bytes.byteLength);
    });
    it("gets binary file with byte-level fidelity", async () => {
      const store = await makeStore();
      const r2 = await store.get("binary.bin");
      globalExpect(r2.bytes.byteLength).toBe(256);
      for (let i = 0; i < 256; i++) globalExpect(r2.bytes[i]).toBe(i);
    });
    if (!opts.skipRange) {
      it("honors range reads", async () => {
        const store = await makeStore();
        if (!store.capabilities?.range) return;
        const r2 = await store.get("binary.bin", { offset: 10, length: 20 });
        globalExpect(r2.bytes.byteLength).toBe(20);
        for (let i = 0; i < 20; i++) globalExpect(r2.bytes[i]).toBe(10 + i);
      });
    }
    it('throws NotFoundError on missing key (name === "NotFoundError")', async () => {
      const store = await makeStore();
      await globalExpect(store.get("does-not-exist.txt")).rejects.toMatchObject({
        name: "NotFoundError"
      });
    });
  });
