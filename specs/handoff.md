...
| `R2Store` (CFW R2Bucket binding) | Done, validated against ctbk's prod bucket. Scoped-`prefixes` empty-prefix list now synthesizes a virtual root (each allowed prefix as a dir) instead of erroring. Optional `presign: { endpoint, bucket, accessKeyId, secretAccessKey, expiresIn? }` enables `getDownloadUrl` (signed) — browser GETs go direct to R2, no worker in the data path. Optional `publicBaseUrl: 'https://<r2.dev or custom-domain>'` enables sync `getUrl` (unsigned, no token, no expiry) for buckets that are already publicly readable — best fit for "public data" projects like ctbk/crashes. |
...
| `S3Store` (S3-compatible API; AWS / R2-via-S3 / MinIO) | Done. SigV4 via `aws4fetch` (small, fetch + `crypto.subtle`; no AWS SDK). Browser-direct or server-proxy (drop-in for `R2Store` in any `createHandlers` callsite). Public (unsigned) and credentialed paths both work. Conformance harness runs via a fake-S3 `fetch` impl backed by `MockStore`. Now a thin wrapper over `_xmlObjectStore` (shared with `GcsStore`). |
| `GcsStore` (Google Cloud Storage, XML/S3-interop API) | Done. Same S3-compat XML wire shape as `S3Store` (both share `_xmlObjectStore` internal). Three auth modes: unsigned (public bucket, exposes `getUrl`), HMAC/SigV4 via GCS interop keys (`getDownloadUrl` presign), OAuth **bearer** via a caller-supplied `getToken()` (no bundled Google auth SDK). Conformance runs 3× (unsigned + bearer + HMAC live-signed against a stubbed global fetch). Bearer auth was live-verified against a real GCS bucket during spec authoring; HMAC/SigV4 region-scope against real GCS still needs a live probe before shipping to production (spec's Phase 0 open item). |
...
| React UI (`src/react/`) | `<FileTree>`, `<DirListing>` (auto-cursor-follow + default-`README.md` panel; in-memory filter by default + opt-in URL state via `usePersistedState`; overridable `filterPlaceholder`), `<TextViewer>` (Range head-fetch + load-all), `<MediaViewer>` (image/video/audio), `<PdfViewer>` (native `<iframe>`, no peer — browser's own PDF chrome), `<Breadcrumb>`, `parsePath`, `makeMatcher` (substring/glob filter), `asyncBufferFromStore` (hyparquet adapter). Pluggable renderer slots (`markdownRenderer`, `parquetRenderer`, `csvRenderer`, `notebookRenderer`, `codeRenderer`, `jsonRenderer`, `pdfRenderer`, `treemapRenderer`); the lib doesn't bundle their deps — consumers wire their renderer of choice. `pdfRenderer` overrides the built-in `<PdfViewer>` (e.g. pdf.js for text-search). When a `treeSource` + `treemapRenderer` are wired the dir view gains a **list / map / split** toggle (`?view=`). |
| URL state (`src/url-state/`) | `useUrlPersistedState` — opt-in hook bridging `use-prms` to the lib's generic `PersistedState` interface. `<FileTree usePersistedState={useUrlPersistedState}>` binds `?q=`, `?page=`, `?json-q=`, `?jq=`. Main entry and renderers don't import `use-prms`; only this sub-path does. |
| Reference renderers (`src/renderers/`) | Done. `parquet`, `markdown`, `csv`, `notebook`, `code`, `json` — all exposed as sub-path exports (`@rdub/file-tree/renderers/<name>`). Heavy peers (`hyparquet`, `react-markdown`, `remark-gfm`, `highlight.js`) are optional. Consumers stop copy-pasting 100+ line files (ctbk/crashes/tomat had all duplicated the parquet ref). |
| Conformance harness (`src/test/conformance.ts`) | Done. 9 tests; pluggable into any new Store impl. |
| Vitest tests | 122 passing across `test/{mock,multi,r2,s3,http,gcs}-store.test.ts`. MultiStore + S3Store + GcsStore all go through full conformance via wrapper / fake-fetch backends. |
...
| `GitHubStore`, `GitLabStore`, `DiskTreeStore` | **TODO** — `GitHubStore` is the natural next, mirrors `S3Store`/`GcsStore` pattern (Bearer-token auth for private repos). |
